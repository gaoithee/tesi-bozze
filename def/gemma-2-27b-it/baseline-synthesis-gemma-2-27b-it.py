...
tokenizer = AutoTokenizer.from_pretrained("google/gemma-2-27b-it", token='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', use_fast = False)
...
temp_df = pd.read_csv('baseline-antithesis-gemma-2-27b-it.csv')
answers = df['thesis']
ant_answers = df['antithesis']
...
for i in range(N_rows):
    syn_answers.append(
        synthesisGeneration(
            first_queries[i], possibilities[i], pre_answers[i], sources[i]))
...
df.to_csv('baseline-final-gemma-2-27b-it.csv')
