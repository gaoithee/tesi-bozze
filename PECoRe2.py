...
df = pd.read_csv("hf://datasets/saracandu/hotpotQA_nli/train.csv")
df = df.dropna()
...
def select_passages(text_passage, question, p, filename, tokens):
    ...
    for i in range(num_of_sets):
        top_scores.extend(top_k_positions(data['cci_scores'][i]['input_context_scores'], 8))
...
def run(question, passage, p, filename):
    if passage != None:
        tokens = tokenizer.tokenize(str(passage))
        invoke_pecore(passage, question, p, filename)
        return select_passages(passage, question, p, filename, tokens)


selected_passages = []
for i in range(len(df)):
    selected_passages.append(run(df['question'][i], df['passages'][i], 5, 'pecore_outs/c' + str(i) + '.json'))
    
df['new'] = selected_passages

df.to_csv('ultramega-test.csv')
